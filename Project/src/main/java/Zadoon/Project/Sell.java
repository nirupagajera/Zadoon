...
public class Sell extends Login
{
	...
	public void Sell_Form() throws Exception  
	{
		...
		while((line = br.readLine())!= null) 
		{
			...
			String checkboxSecond = (FormData.length> checkbox2)? FormData[checkbox2].trim() : " ";
		        	...
		        	for(WebElement value : Value) 
		        	{
		        		String name = value.getAccessibleName();
 			        		if(name.equalsIgnoreCase(are_you_Open_Path))
			        		{
			        			...
			        			if(name.equalsIgnoreCase(checkboxSecond))  {
			        			value.click();
			        			break;
			        			}
